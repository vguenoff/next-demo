...
The React team has long said that Suspense is a low-level feature, something built into libraries and meta-frameworks. Open-source maintainers need to learn the hard stuff, but not app developers like us.
...
- Creating a Suspense boundary with `<React.Suspense>`.
...
Server Components are also allowed to be asynchronous. This means that when we render the component that performs an asynchronous action (e.g. fetch data), we get back a Promise, and that promise will resolve after the data-fetching is complete and the component has been rendered.
...
[Next.js docs on Loading UI and Streaming](https://nextjs.org/docs/app/building-your-application/routing/loading-ui-and-streaming)
