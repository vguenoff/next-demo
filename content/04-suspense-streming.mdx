...
We know that Server Components can also be asynchronous. This means that when we render the component that performs an asynchronous action (e.g. fetch data), we get back a Promise, and that promise will resolve after the data-fetching is complete and the component has been rendered. And **with Server Components, React can use the component itself to track whether the component is ready or not**.
