...
So TrendingProducts is a React component, but it's also an async function?? What's going on here?
...
- **Client Components** are the components we've been working already in pevious versions of React and Next.
...
RSC are a significant paradigm shift, an entirely new way to think about components!

The most obvious benefit is performance. If half of the components in your application are Server Components, it means that your JS bundle will be significantly smaller, since all of that code can be omitted.
...
Our top-level there is a Server Component, and it "owns" both the ClientComponent and ServerComponent components. This is the relationship that matters!
